...
| Browse and sources | Partial | Popular/latest, paging, search, filters, preferences, per-source persisted grid/list display, catalogue item menus, batch migration with seasonal matching/comparison/options/preview/progress/recovery, installed-extension metadata and details, durable language filters, pinned ordering, update badges/actions, source/extension overflow menus, fixed row density, and retryable loading/error states | Public repository compatibility validation |
