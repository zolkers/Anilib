...
| Network maintenance | Partial | Shared cookies/cache/rate limits plus confirmed Android/desktop actions to clear HTTP cookies, WebView cookies, and cached responses | Default user agent, proxy/DoH policy, remaining WebView cache/storage, unused database cleanup, and diagnostics |
| WebView | Partial | Source API 1.5 source/title entry points, Android System WebView, desktop KCEF, navigation controls, and bidirectional shared-cookie handoff | CAPTCHA validation, file chooser/pop-up/download handling, per-source browser policy, and complete WebView data clearing |
