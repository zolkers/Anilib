...
| Browse and sources | Partial | Popular/latest, paging, search, filters, preferences, per-source persisted grid/list display, catalogue item menus, migration, installed-extension metadata and details, durable language filters, pinned ordering, update badges/actions, source/extension overflow menus, fixed row density, and retryable loading/error states | Migration depth |
