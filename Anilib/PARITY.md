...
| Legacy Aniyomi extensions | Partial | Android-only HTTPS APK download, bounded redirect handling, unknown-source permission flow, user-confirmed PackageInstaller hand-off, and metadata discovery for Android-visible installed extensions | Supply the external Aniyomi host ABI needed to execute discovered APK sources without leaking it into shared modules; portable Anilib bundles remain required for desktop |
