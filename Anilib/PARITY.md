...
| APK extensions (Android) | Partial | HTTPS APK download, PackageInstaller hand-off, visible-package discovery, certificate trust, host-ABI preflight, and pre-start adaptation of ABI-ready anime catalogues, episodes, streams, headers, and subtitles into Source Bundles | Supply the complete external host ABI, add manga APK compatibility, and finish filters/preferences plus modern suspend/hoster APIs; portable Anilib bundles remain the active Android/desktop format |
