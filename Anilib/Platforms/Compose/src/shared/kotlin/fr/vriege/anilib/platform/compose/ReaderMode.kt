 ...
 *
 *  - [ReaderPagedMode] (left-to-right, right-to-left) shows one page at a time; up/down turn pages,
 *    and horizontal keys follow the reading direction.
 *  - [ReaderVerticalMode] snaps through the same chapter window one page at a time on the vertical
 *    axis, with gesture and keyboard navigation owned by its dedicated pager.
...
internal sealed interface ReaderMode {
    ...
    /** True when pages snap one viewport at a time on the vertical axis. */
    val verticalPager: Boolean
    ...
    companion object {
        fun of(direction: ReadingDirection): ReaderMode = when (direction) {
            ...
            ReadingDirection.VERTICAL -> ReaderVerticalMode
